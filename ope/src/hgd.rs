...
pub fn afc(index: &u32) -> f32 {
    ...
    match index.cmp(&1) {
        ...
        Ordering::Greater => {
            ...
            (index + 0.5) * index.ln() - index + frac_12 / index
                - frac_360 / index / index / index
                + frac_pi
        }
    }
}
...
impl HGD {
    ...
    fn hypergeometric_hrua(prng: &PRNG, good: &f64, bad: &f64, sample: &f64) -> f64 {
        ...
        let d7: f64 =
            ((popsize - m) * *sample * d4 * d5 / (popsize - 1_f64) + 0.5).sqrt();
        let d8: f64 = D1 * d7 + D2;
        let d9: f64 = (m + 1_f64) * (mingoodbad + 1_f64) / (popsize + 2_f64);
        let d10: f64 = HGD::loggam(d9 + 1_f64)
            + HGD::loggam(mingoodbad - d9 + 1_f64)
            + HGD::loggam(m - d9 + 1_f64)
            + HGD::loggam(maxgoodbad - m + d9 + 1_f64);
        ...
        loop {
            ...
            let t: f64 = d10
                - (HGD::loggam(z + 1.0)
                    + HGD::loggam(mingoodbad - z + 1.0)
                    + HGD::loggam(m - z + 1.0)
                    + HGD::loggam(maxgoodbad - m + z + 1_f64));
    ...
    fn loggam(x: f64) -> f64 {
        ...
        let a: Vec<f64> = vec![
            ...
            6.410_256_410_256_410e-03,
            -2.955_065_359_477_124e-02,
            1.796_443_723_688_307e-01,
            -1.392_432_216_905_900e+00,
        ];
...
mod tests {
    ...
    fn test_rhyper() {
        ...
        for i in 1..=10 {
            assert_eq!(
                HGD::rhyper(&(i as f64), &2_f64, &3_f64, &coins),
                HGD::hypergeometric_hyp(&prng, &2_f64, &3_f64, &(i as f64))
            );
        }

        assert_eq!(
            HGD::rhyper(&11_f64, &20_f64, &20_f64, &coins),
            HGD::hypergeometric_hrua(&prng, &20_f64, &20_f64, &11_f64)
        );
    }
